...
class bagging_classifier:
    ...
    def fit(self, X, y, eval_set=None, eval_metric=None, verbose=None, early_stopping_rounds=None):
    ...
    def predict_proba(self, X):
        ...
        for estimator in self.estimators_:
            proba += estimator.predict_proba(X, ntree_limit=estimator.get_booster().best_iteration) / self.n_estimators_
...
for n_fold, (trn_idx, val_idx) in enumerate(folds.split(data, data['TARGET'])):
    ...
    fold_importance_df["importance"] = clf.feature_importances
...
plt.legend()
prob_df.to_csv(f'{current_path}/../../output/train_pred_xgb1.csv', index=False)

test['TARGET'] = sub_preds
test[['SK_ID_CURR', 'TARGET']].to_csv(f'{current_path}/../../output/test_pred_xgb1.csv', index=False, float_format='%.8f')
